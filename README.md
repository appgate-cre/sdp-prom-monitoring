...
This is a fork and was specialized to fit the Appgate training environment Trent. 

## Quick run

The following steps apply to a monitoring unit on AWS for trent:

1. ssh into box, then: `sudo yum -y  install git && git clone https://github.com/appgate-cre/sdp-prom-monitoring.git`
1. `prep-amazon-linux.sh`
1. `prep-services.sh`
1. Get the prometheus job config and connections config from `trent/prepare.py`
	1. `cat >> prometheus/prometheus.yml # append the job config`
	1. `cat > conkolla/connections.yml  # overwrite the connections config`
1. Assure the hostnames are set in the DNS pointing to this box.
1. `HOSTNAME=$(hostname) docker stack deploy -c docker-appgate-monitor-stack.yml agmon`
...
```shell
git clone https://github.com/appgate-cre/appgate-prom-monitoring.git
```
